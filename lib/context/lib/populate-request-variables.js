  ...
  return function(ctx, req, callback) {
    ...
    function setRequestBody(error) {
      if (error) {
        ...
        ctx.set('error.status.code', error.statusCode || 500);
        error = {
          name: 'RuntimeError',
...
function getParserSizeLimit() {
  var projectInfo = apicConfig.inspectPath(process.env.CONFIG_DIR || process.cwd());
  ...
  [ apicConfig.PROJECT_STORE, apicConfig.USER_STORE].some(function(location) {
    ...
    if (obj.parserSizeLimit && _.isInteger(parseInt(obj.parserSizeLimit))) {
