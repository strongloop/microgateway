...
var logger = require('../../../../apiconnect-cli-logger/logger.js')
               .child({loc: 'apiconnect-microgateway:context:populate-request-variables'});
...
module.exports = function populateRequestVariables(options) {
  ...
  logger.debug('bodyParser option: ', bodyParserOption);
  ...
  return function(ctx, req, callback) {
    ...
    ctx.set('request.parameters', parseParameters(req.originalUrl));
      ...
      if (typeis.hasBody(req)) {
        switch (bodyFilterOption[req.method]) {
          case filterReject:
            ...
            return;
          case filterIgnore:
            logger.debug('Ignore payload from ' + req.method + ' request');
            break;
          default:
            logger.debug('use ' + parserName + 'Parser to parse payload');
            bodyParser[parserName](req, {}, setRequestBody);
            return;
        }
      }

      logger.debug('request does not have payload');
      switch (parserName) {
        case 'json':
          setRequestBody({
            name: 'PopulateContextError',
    ...
    function setRequestBody(error) {
      if (error) {
        logger.debug('parse payload error ', error);
  ...
  function getPath(originalUrl, basepath) {
    ...
    assert(originalUrl && originalUrl.charAt(0) === '/');

    // always start at 1, since `originalUrl` will always include `/`
    var indexStart = 1, indexEnd = originalUrl.length;

    if (basepath.length > 0) {
      logger.debug('basepath is "' + basepath + '"');

      if (originalUrl.indexOf(basepath) < 0) {
        logger.debug('url "' + originalUrl + '" does not start with basepath');
      }
    }

    var queryParamIndex = originalUrl.indexOf('?');
    if (queryParamIndex > 0) {
      logger.debug('trim query parameters after ' + queryParamIndex + 'th char');
  ...
  /**
   * Return the parameters object associated with the request
   */
  function parseParameters(url) {
    // TODO parse according to the API swagger parameters definition
    // The following implementation is a temp solution for demo purpose
    // At the end, we'll need to parse according to swagger
