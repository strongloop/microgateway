  ...
  return new Promise(function(resolve, reject) {
    ds.start(process.env.NODE_ENV === 'production')
      .then(function(useHttps) {
        var port = process.env.PORT || (useHttps ? 443 : 80);
        logger.debug ('starting gateway ', port);
        if (useHttps) {
          if (!process.env.TLS_SERVER_CONFIG)
            process.env.TLS_SERVER_CONFIG = './config/onPremEssentialsTLS.json';
            ...
            if(options[file]) {
              if(Array.isArray(options[file])) { // ca is capable of being an array
                ...
                try {
                  ...
                  var potentialFile = path.join(dirname, filename);
                  stats = fs.statSync(potentialFile);
                  if (stats.isFile()) {
...
if (require.main === module) {
  exports.start().
    then(function(port) {
      logger.debug('micro-gateway listening on port ' + port);
    });
}
