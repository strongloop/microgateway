...
module.exports = function(config) {
  ...
  return function(req, res, next) {
    ...
    if (_.isString(query['confirmation'] || body['confirmation']) && 
        _.isString(query['username'] || body['username']) &&
        _.isString(query['app-name'] || body['app-name'])) {
      //back from redirect
    ...
    } else if(_.isString(query['dp-state'] || body['dp-state']) &&
        _.isString(query['error'] || body['error'])) {
      //back from redirect error
      req.oauth2 = { type: 'AZ redirect error' };
      logger.debug('Receiving an OAuth2 AZ redirect error request.');
...
ServerPool.prototype.getServer = function (apiid, apidoc, snapid) {
  ...
  if (_.isUndefined(server)) {
    ...
    if (idExtract.type === 'basic') {
      azHandlers.push(
          idExtractor({apidoc: apidoc, sessionKey: SESSION_KEY, form: false}));
      ...
      var redirectHandler = redirect(
          {
            redirectURL: idExtract['redirect-url']
          });
          ...
          helpers.validateToken(_this.models, token, 'refresh-token',
            function(err, saved) {
              ...
              _this.models.refreshTokens.delete(
                      clientId, saved.id, function(err, result) {
