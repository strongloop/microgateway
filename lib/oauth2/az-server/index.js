...
var defaultForm = require('./id-extract/default-form');
...
module.exports = function(config) {
  ...
  return function(req, res, next) {
    ...
    if (req.ctx.request.body['transaction_id'] || (
        _.isString(parameters['response_type']) &&
      ...
      if (req.ctx.request.verb === 'GET') {
        ...
        //login form or consent form
        var username = req.ctx.request.body['j_username'];
        var passwd = req.ctx.request.body['j_password'];
        if (_.isString(username) && _.isString(passwd)) {
          //handle login form
        ...
        next ({name: 'OAuth2Error', message: 'Unsupported verb:' + req.ctx.request.verb});
      }
...
ServerPool.prototype.getServer = function (apiid, apidoc, snapid) {
  ...
  if (_.isUndefined(server)) {
    ...
    if (authorizationType.type === 'authenticated') {
      ...
      azHandlers.push(checkAuthorizeHandler, decisionViewHandler);
      loginHandlers.push(checkAuthorizeHandler, decisionViewHandler);
    }
  ...
  return function (req, res, next) {
    if (req.oauth2.client.logined && req.oauth2.client.logined === true) {
      ...
      server._respond(req.oauth2, req.ctx, function(err) {
        ...
        return next(new AuthorizationError(
                    'Unsupported response type: ' + req.oauth2.req.type,
