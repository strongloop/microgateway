...
module.exports = function(config) {
  ...
  defaultFormMiddleware = defaultForm();
...
ServerPool.prototype.getServer = function (apiid, apidoc, snapid) {
  ...
  if (_.isUndefined(server)) {
    ...
    if (idExtract.type === 'basic') {
      ...
      loginHandlers.push(this.sessionMiddleware);
      loginHandlers.push(transactionLoader(server));
      loginHandlers.push(basicAuthHandler);
    }
    ...
    if (authorizationType.type === 'authenticated' && 
      ...
      azHandlers.push(authorizationAuthenticatedHandler(server));
    } else {
      if (idExtract.type === 'default-form') {
        azHandlers.push(defaultFormMiddleware);
      }
      azHandlers.push(
        isAuthorizedHandler(server, _this.models),
...
function authorizationAuthenticatedHandler(server) {
  return function (req, res, next) {
    if (req.oauth2.client.logined && req.oauth2.client.logined === true) {
      ...
      return next(new AuthorizationError('Authenticated failed'));
    }
  };
}
