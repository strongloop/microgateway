...
function getTokenHandler(jwtToken, done) {
  return function(err, savedToken) {
    if (err || !savedToken) {
      return done(err);
    }

    done(null, savedToken.id, helpers.buildTokenParams(savedToken, jwtToken));
  };
}
    ...
    if (enableRefreshToken) {
      server.exchange(oauth2Provider.exchange.refreshToken(
        function(client, refreshToken, scope, done) {
          ...
          //TODO: find by token id not by token
          _this.models.accessTokens.findByRefreshToken(refreshToken,
            function(err, accessToken) {
              ...
              _this.models.accessTokens.save(apiid, clientId, accessToken.userId,
