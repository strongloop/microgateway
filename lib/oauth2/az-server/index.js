...
ServerPool.prototype.getServer = function (apiid, apidoc, snapid) {
  ...
  if (_.isUndefined(server)) {
    ...
    if (authorizationType.type === 'authenticated') {
      ...
      azHandlers.push(notImplemented);
      loginHandlers.push(notImplemented);
    }
...
/*
 * reject the request with not supported error
 */
function notImplemented(req, res, next) {
  logger.error('custom consent form is not supported yet');
  return next(new AuthorizationError('Not supported', 'server_error'));
}
