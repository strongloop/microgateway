      ...
      server.grant(oauth2Provider.grant.token(
        ...
        function(client, user, scope, ares, done) {
          ...
          function generateAccessToken() {
            ...
            _this.models.accessTokens.save(apiid, token.id, client['client-id'], user.id, scope, 
                null, tokenTTL, token.secret, getTokenHandler(token, done));
          }
        ...
        function(client, scope, done) {
          ...
          function generateAccessToken(user) {
            ...
            _this.models.accessTokens.save(
              apiid, token.id, client.id, user && user.id, scope, refreshToken,
              tokenTTL, token.secret, getTokenHandler(token, done));
          }
        ...
        function(client, username, password, scope, done) {
          ...
          server.userAuthentication(snapid, authCfg, username, password,
            function(err, user) {
              ...
              _this.models.accessTokens.save(apiid, token.id, client.id, user.id,
                scope, refreshToken, tokenTTL, token.secret, getTokenHandler(token, done));
        ...
        function(client, refreshToken, scope, done) {
          ...
          _this.models.accessTokens.findByRefreshToken(refreshToken,
            function(err, accessToken) {
              ...
              _this.models.accessTokens.save(apiid, token.id, client.id, accessToken.userId,
                scope, refreshToken, tokenTTL, token.secret, getTokenHandler(token, done));
            });
        }));
    }
