...
function clientAuthenticate(clientType, authCfg, models) {
  return function (req, res, next) {
    ...
    var secretIsNotRequired =
        (grantType === 'password' && clientType === 'public') || false;

    if (!client_id || (!client_secret && !secretIsNotRequired)) {
    ...
    models.clients.findById(snapshotId, client_id, req.ctx._.api.id,
      function(err, result) {
        if (result) {
          if (secretIsNotRequired || result['client-secret'] == client_secret) {
