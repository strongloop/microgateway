    ...
    if (req.oauth2.res && req.oauth2.res.allow === true &&
      ...
      ctx.message.body = contentFn(
          { transaction_id: oauth2.transactionID,
            ...
            resource_owner: encodeURIComponent(oauth2.user.id),
            redirect_uri: encodeURIComponent(oauth2.req.redirectURI),
            original_url: encodeURIComponent(actionURI),
            client_id: encodeURIComponent(oauth2.req.clientID),
            scope: oauth2.req.scope,
            scopeStr: encodeURIComponent(oauth2.req.scope.join(' ')),
            dp_data: ''});
      //reset all headers
