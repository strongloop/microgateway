...
module.exports = function token(server, options) {
  ...
  if (!server)
      throw new TypeError(
              'oauth2orize.token middleware requires a server argument');

  return function token(req, res, next) {
    var type = req.ctx.request.parameters.grant_type;

    server._exchange(type, req, function(err) {
      if (err)
          return next(err);

      var tknError = new TokenError(
              'Unsupported grant type: ' + type,
              'unsupported_grant_type');
      return next(tknError);
    });
  };
};
