    ...
    server._parse(type, req, function(err, areq) {
      ...
      if (!areq || !Object.keys(areq).length) {
        return next(new AuthorizationError(
          'Missing required parameter: response_type', 'invalid_request'));
      }
      if (Object.keys(areq).length === 1 && areq.type) {
        return next(new AuthorizationError(
            'Unsupported response type: ' + type, 'unsupported_response_type'));
      }
      ...
      function validated(err, client, redirectURI) {
        ...
        if (!client) {
          return next(new AuthorizationError('Invalid client',
            'invalid_client'));
        }

        var validURI = client['oauth-redirection-uri'];
        if (!redirectURI) {
        ...
        var txn = helpers.searchTXData(txns, areq) || helpers.createTXData(client);

        //restore data from session
        req.oauth2 = txn;

        //check session information to see if user login or not
        if (_.isUndefined(txn.client.logined) || txn.client.logined === false) {
          ...
          txn.redirectURI = redirectURI || client['oauth-redirection-uri'];
          txn.req = areq;
          // store transaction in session
          txns[txn.transactionID] = txn;
        }
        next();
      }
      ...
      try {
        ...
        if (arity === 4) {
          validate(req.ctx, areq.clientID, areq.redirectURI, validated);
        } else if (arity === 5) {
          validate(req.ctx, areq.clientID, areq.redirectURI, areq.scope, validated);
        } else if (arity === 6) {
          validate(req.ctx, areq.clientID, areq.redirectURI, areq.scope, areq.type, validated);
        } else { // arity == 3
          validate(req.ctx, areq, validated);
        }
      } catch (ex) {
        return next(ex);
      }
