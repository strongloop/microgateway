...
Server.prototype.grant = function(type, phase, fn) {
  if (typeof type === 'object') {
    ...
    if (mod.responseType) { this.responseType(mod.responseType); };
    return this;
  }
  ...
  if (phase == 'request') {
    logger.debug('register request parser %s %s', type || '*', fn.name || 'anonymous');
    this._reqParsers.push({ type: type, handle: fn });
  } else if (phase == 'response') {
    logger.debug('register response handler %s %s', type || '*', fn.name || 'anonymous');
...
function(snapid, authCfg, username, password, done) {
  return authentication.userAuthenticate(snapid, authCfg, username, password, done);
};
 ...
 * @api public
 */
Server.prototype.authentication = function(authCfg, models) {
  return authentication.clientAuthenticate(authCfg, models);
};
