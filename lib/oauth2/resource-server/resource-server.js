...
function setupResourceServer(app, options, models) {
  ...
  function jwtAccessTokenValidator (req, jwtToken, done) {
    ...
    //   iat: iat // createdAt
    // }
    var accessToken = JSON.parse(decoded.payload);

    logger.debug('Received accessToken:', accessToken);
    ...
    models.accessTokens.find(accessToken.jti, function (err, token) {
      if (err || !token) {
        return done(err || new TokenError('Access token not found'));
      }

      logger.debug('Access token found: %j', token);

      if (!jwt.verify(jwtToken, 'HS256', token.secret))
        return done(new TokenError('Access token cannot be verified', 'invalid_grant'));
        ...
        function lookupUser () {
          return new Promise(function (resolve, reject) {
            ...
            models.users.find(userId, function (err, user) {
              if (err || !user) {
                err = err ||
                  new TokenError('Access token has invalid user id: ' + userId, 'invalid_grant');
                reject(err);
              }
        ...
        function lookupApp () {
          return new Promise(function (resolve, reject) {
            ...
            models.clients.find(appId, function (err, app) {
              if (err || !app) {
                err = err ||
                  new TokenError('Access token has invalid app id: ' + appId, 'invalid_grant');
                reject(err);
              }
  ...
  var jwtBearerStrategy = new BearerStrategy(
    ...
    //  try {
    //    jwtAccessTokenValidator(req, jwtToken, done);
    //  }
    //  catch (err) {
    //    console.log(err);
    //  }
