...
function setupResourceServer(app, options, models) {
  ...
  function decodeToken (jwtToken) {
    ...
    if (!decoded || !decoded.payload) {
      logger.debug('Could not decode token: %j', jwtToken);
      return new AuthorizationError('Could not decode token', 'invalid_request');
    }
    var token;
    try {
      ...
      logger.debug('Token payload could not be parsed: %j', jwtToken);
      return new AuthorizationError('Token payload could not be parsed', 'invalid_request');
    }
  ...
  function jwtAccessTokenValidator (req, jwtToken, done) {
    ...
    if (formatError) {
      logger.debug('Access token has invalid format (%s): %j', formatError.message, accessToken);
    ...
    if ((new Date(accessToken.exp)) < (new Date())) {
      ...
      return done(new AuthorizationError('Access token is expired', 'access_denied'));
    }

    models.accessTokens.find(req.oauth2.apiId, accessToken.jti, function (err, token) {
      if (err) {
        logger.error(err);
      ...
      if (!token)
        return done(new AuthorizationError('Access token not found', 'access_denied'));

      logger.debug('Access token found: %j', token);

      if (!jwt.verify(jwtToken, 'HS256', token.secret))
        return done(new AuthorizationError('Access token cannot be verified', 'access_denied'));

      var matchErr = compareAccessToken(accessToken, token);
      if (matchErr) {
        logger.debug('Token mismatch error (%s): %j %j', matchErr.message, accessToken, token);
        return done(new AuthorizationError('Token mismatch', 'access_denied'));
      }
      ...
      if (!scopesSatisfied(req.oauth2.requiredScopes, token.scopes)) {
        logger.debug('Token does not satisfy scopes required by API');
