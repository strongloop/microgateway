...
module.exports = function(config) {
  ...
  return function(ctx, descriptor, securityReq, securityDef, filters, callback) {
    ...
    */
    var authenticators = rs.authenticate(function (err, user, token) {
      if (err || /Bearer realm/.test(token)) {
        err = err || { status: 401, code: 'invalid_token' };
    ...
    req.oauth2 = {
      requiredScopes: _.get(_.toPairs(securityReq), '[0][1]')
    };
    var res = {};
