  ...
  if (options.redirectURI) {
    ...
    if (!matched) {
      err = new AuthorizationError(
          'Unregisterd redirect URI "' + options.redirectURI + '"',
          'invalid_client');
      return next(err) || err;
    }
  }
...
function deleteRefreshToken(models, token, done) {
  ...
  if (tmp) {
    ...
    if (jwtTkn.jti) {
      return models.refreshTokens.delete(
              undefined, jwtTkn.jti, function(err, record) {
        if (err || !record) {
...
function deleteAZCode(models, code, done) {
  console.log('Error! The deleteAZCode() is not yet implemented.');
  return done('Error! The deleteAZCode() is not yet implemented.');
}
 ...
 * done callback.
 */
function validateToken(models, token, type, done) {
  var tmp = token.split('.')[1];
  ...
  if (tmp) {
    ...
    if (jwtTkn.jti) {
      ...
      if (table) {
        return table.find(jwtTkn.jti, function(err, record) {
          if (err || !record)
            return done(err, record);
...
function redirectError(ctx, oauth2, err) {
  ...
  if (oauth2.req) {
    if (oauth2.req.redirectURI) {
      redirectURI = oauth2.req.redirectURI;
    }
    if (oauth2.req.state) {
  ...
  } else if (oauth2.client && oauth2.client['oauth-redirection-uri']) {
    redirectURI = oauth2.client['oauth-redirection-uri'];
  }
  ...
  if (state) { uri.query.state = state; }
...
function getRedirectURI(ctx, oauth2) {
  if (oauth2.req && oauth2.req.redirectURI) {
    return oauth2.req.redirectURI;
  }
  ...
  if (oauth2.client && oauth2.client['oauth-redirection-uri']) {
    return oauth2.client['oauth-redirection-uri']
  }
