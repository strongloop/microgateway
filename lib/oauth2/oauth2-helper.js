...
function buildTokenParams(savedToken, jwtToken) {
  var params = {
    expires_in: savedToken.expiresIn
  };

  var scope = savedToken.scopes && savedToken.scopes.join(' ');
  if (scope) {
    params.scope = scope;
  }

  if (savedToken.refreshToken) {
    params.refresh_token = savedToken.refreshToken;
  }

  if (typeof jwtToken === 'object') {
    for (var p in jwtToken) {
      if (p !== 'id' && !params[p] && jwtToken[p]) {
        params[p] = jwtToken[p];
      }
    }
  }
