...
function initialize(oauthModels, options) {
  ...
  var accessToken = {};
  accessToken.find = function(apiId, accessToken, done) {
    logger.debug('accessToken.find(<token>)');
  ...
  accessToken.delete = function(apiId, clientId, token, tokenType, done) {
    logger.debug('accessToken.delete(', clientId, ',<token>,', tokenType, ')');
    var where = {
      apiId, apiId,
      appId: clientId
    };
  ...
  accessToken.save = function(apiId, token, clientId, resourceOwner, scopes, 
