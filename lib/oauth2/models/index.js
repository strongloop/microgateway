...
var logger = require('apiconnect-cli-logger/logger.js')
  .child({loc: 'microgateway:oauth:az-server:models'});
...
var defs = {
    ...
    OAuthAccessToken: require('../common/models/oauth-token.json')
};
...
function OAuthModels (config) {
  this.dbName = 'microgw-oauth-db';
  var dataSourceDef = {name: this.dbName, connector: 'memory'};
  config = config || {};

  if (config && config.dataSource !== undefined) {
    dataSourceDef = config.dataSource;
    this.dbName = dataSourceDef.name || 'microgw-oauth-db';
  }
...
function initialize(oauthModels, options) {
  ...
  console.error(applicationModel);
...
module.exports.OAuthModels = OAuthModels;