...
function initialize(oauthModels, options) {
  ...
  token.purge = function() {
    var where = {
      expiredAt: { lte: new Date() }
    };

    oAuthTokenModel.destroyAll(where, function(error, result) {
      if (error) {
  ...
  token.save = function(token, clientId, resourceOwner, scopes, refreshToken, expire, done) {
    var tokenObj;
  ...
  code.purge = function(done) {
    ...
    oAuthAuthorizationCodeModel.destroyAll(where, function(error, result) {
      if (error) {
        logger.error('Failed to purge the authorization code.', error);
        return;
      }
      logger.debug('%d expired authorization code are purged', result.count);
    });
  };
