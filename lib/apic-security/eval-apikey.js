...
function matchSubscriptionByClientId(ctx, candidate, clientId) {
  var snapshotId = candidate.doc.snapshotId;
  var docId = candidate.doc['x-ibm-api-id'];
  var subs = ctx.dataStore.models.subscription.matchByClientId(snapshotId, docId, candidate.path, candidate.method, clientId);
  candidate.subscription = subs[0].subscription;
  candidate['app-credential'] = subs[0]['app-credential'];
  return subs.length > 0;
}

function evalApikey(ctx, candidate, securityReq, securityDef, callback) {
  var descriptor = candidate.doc;
  ...
  if (securityDef.in === 'header') {
    ...
    if (securityDef.name === 'X-IBM-Client-Id') {
      var clientId = ctx.request.headers['x-ibm-client-id'];
      if (!clientId) {
        return callback(false);
      }
      callback(matchSubscriptionByClientId(ctx, candidate, clientId));
    }
