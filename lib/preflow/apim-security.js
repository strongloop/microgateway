...
function evalBasic(tx, context, securityReq, securityDef, filters, callback) {
  ...
  if (authurl.includes('ldap://')) {
    dsc.getCurrentSnapshot()
      ...
      .then(reg => {
        // TODO jcbelles: stop cheating here
        if (!(reg && reg[0] && reg[0]['ldap-config']))
          throw new Error(`Missing LDAP Config: ${JSON.stringify(reg, null, '  ')}`);
        const conf = reg[0]['ldap-config'];
        conf.host = 'localhost';
        conf.port = authurl.split(':')[2];
        conf['authenticated-bind'] = true;
        conf['authenticated-bind-admin-dn'] = 'cn=root';
        conf['authenticated-bind-password'] = 'secret';
        conf['search-filter'] = '(cn={{username}})';
        return BasicLdap(reg[0]);
      })
      .then((ldapauth) => {
