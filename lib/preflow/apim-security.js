...
function evalBasic(tx, context, securityReq, securityDef, filters, callback) {
  ...
  if (authurl.includes('ldap://')) {
    dsc.getCurrentSnapshot()
      .then(id => dsc.getRegistry(id, 'ldap1'))
      .then(reg => {
        // FIXME jcbelles: this probably needs its own function in basic-ldap, it's going to get complicated...
        let ldapconf = (reg && reg[0] && reg[0]['ldap-config']) || {};
        return {
          url: authurl, // TODO jcbelles: we should probably actually use ldapconf.host or at least verify they match...
          bindDn:          ldapconf['authenticated-bind-admin-dn'] || 'cn=root',
          bindCredentials: ldapconf['authenticated-bind-password'] || 'secret',
          searchBase:      ldapconf['search-dn-base'] || 'ou=myorg,ou=com',
          searchFilter:    ldapconf['search-filter'] || '(cn={{username}})'
        }
      })
      .then((ldapconf) => {
        let ldapauth = require('./basic-ldap')(ldapconf);
