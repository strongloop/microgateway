...
function evalBasic(tx, context, securityReq, securityDef, filters, callback) {
  ...
  if (typeof authurl !== 'string') {
    logger.debug(`evalBasic error: Security definition provided invalid authentication URL: ${authurl}`);
    callback(false);
    return;
  }
  ...
  if (authurl.includes('ldap://') || authurl.includes('ldaps://')) {
    ...
    if (typeof authreg !== 'string') {
      logger.debug(`evalBasic error: Security definition provided invalid authentication registry: ${authreg}`);
      callback(false);
      return;
    }
    dsc.getRegistry(context['snapshot-id'], authreg)
      .then(registries => {
        let ldapregs;
        ...
        if (config.registry['ldap-config'].ssl) {
          ...
          return dsc.getTlsProfile(context['snapshot-id'], tls).then(tlsprofile => {
...
function evalSecurity(tx, context, securityReqs, securityDefs, filters, callback) {
  ...
  logger.debug('evalSecurity context:', context);
