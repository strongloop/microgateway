...
function basicAuth(snapshotId, authCfg, authObj, done) {
  ...
  if ((authObj && authObj.scheme) !== 'Basic') {
    ...
    return done({
      error: 401,
      message: 'Basic authorization data not found' });
  }
  ...
  if (authreg) {
    if (typeof authreg !== 'string') {
      ...
      return done({
        error: 500,
        message: 'Security definition provided an invalid auth registry.' });
    }
  ...
  if (!authurl || typeof authurl !== 'string') {
    ...
    return done({
      error: 500,
      message: 'Security definition provided an invalid auth URL.' });
  }
  ...
  if (/^ldaps?:\/\//.test(authurl)) {
    ...
    return done({
      error: 500,
      message: 'LDAP protocol is not supported yet' });
    ...
    return done({
      error: 500,
      message: 'Unsupported basic authentication configuration' });
  }
}
  ...
  return function(err, res) {
    if (!err) {
      if (res.statusCode < 400) {
        ...
        return done({ error: res.statusCode, message: 'Basic Auth failed' });
      }
    }

    logger.error('Basic Auth failed with unexpected error', err);
    return done({ error: 500, message: err });
  };
}
  ...
  if (tlsProfileName) {
    dsc.getTlsProfile(snapshotId, tlsProfileName).then(function(tlsprofile) {
      if (Array.isArray(tlsprofile) && tlsprofile.length >= 1) {
        ...
        return done({ error: 500, message: 'Server error' });
      }
    })
    .catch(function(error) {
      done({ error: 500,
        message: error || 'Error when getting TLS Profile' });
    });
  } else {
  ...
  dsc.getRegistry(snapshotId, authreg)
    .then(function(registries) {
      if (!Array.isArray(registries) || registries.length < 1) {
        throw {
          error: 500,
          message: 'Authentication registry (' + authreg + ') is not found' };
      }
      ...
      if (userReg.length < 1) {
        throw {
          error: 500,
          message: 'User registry (' + authreg + ') is not found' };
      }
      if (userReg.length > 1) {
      ...
      if (regCfg.type === 'ldap') {
        if (userReg[0]['ldap-config'].ssl) {
          ...
          if (!tls) {
            throw {
              error: 500,
              message: 'No TLS profile is provided for the secured LDAP registry' };
          }
          ...
          return dsc.getTlsProfile(snapshotId, tls).then(function(tlsprofile) {
            if (!Array.isArray(tlsprofile) || tlsprofile.length < 1) {
              throw {
                error: 500,
                message: 'TLS profile (' + tls + ') is not found' };
            }
        ...
        throw {
          error: 500,
          message: 'The registry of "' + regCfg.type + '" is not supported yet' };
      }
    })
    .catch(function(err) {
...
function ldapBasicAuth(authObj, ldapCfg, done) {
  return Promise.resolve(BasicLdap(ldapCfg))
    ...
    .then(function(user) {
      ...
      return done({ error: 500, message: 'Unexpected error in LDAP basic auth.' });
    })
    .catch(function(err) {
      if (err && (err.code === 49 || err.code === 32)) {
        ...
        return done({ error: 401, message: 'Invalid user/credential' });
      } else {
        logger.error('LDAP error:', err);
        return done({ error: 500, message: 'LDAP error' });
      }
    });
}
