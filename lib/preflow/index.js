...
function runRateLimiters(api, ctx) {
  return new Promise(function(resolve, reject) {
    ...
    if (limited) {
      var tasks = api.rateLimits.filter(function(rateLimit, index) {
...
function filterAPI(req, res, ctx, apis) {
  ...
  var badTestData = apis.filter(function(_api) { return _api.badTestData; }).length > 0;
  apis = apis.filter(function(_api) { return _api.authenticated; });
  if (apis.length < 1) {
    var status = ctx.get('error.status.code');
    if (!status || status < 400) {
      ctx.set('error.status.code', badTestData ? 403 : 401);
    }
    return null;
  }
