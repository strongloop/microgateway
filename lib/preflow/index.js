    ...
    contextget(tx, contextgetOptions, function(error, apis) {
      if (error || !apis) {
        ...
        if (!api) {
          ...
          const ibmExtension = api.swagger['x-ibm-configuration'];
          if (ibmExtension && ibmExtension.cors && ibmExtension.cors.enabled) {
            res.set('Access-Control-Allow-Origin', '*');
          }
        }
      }

      // if there is no error, then error === undefined.
      // next(undefined) will continue with the normal middleware chain
      next(error); 
