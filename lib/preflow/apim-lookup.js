...
function evalApikey(ctx, context, securityReq, securityDef, filters, callback) {
  logger.debug('evalApikey entry:',
        '\n   in:', securityDef.in,
        '\n   name:', securityDef.name);/*, sensitive data
 ...
 */
 function findContext(ctx, filters, body, contextsCB) {
     ...
     promise = promise.then(function() {
       return new Promise(function(resolve, reject) {
         security.evalSecurity(ctx, mc.entry, reqs, defs, filters, function(err, result) {
           ...
           var rateLimits = mc.mm['observed-rate-limit'];
           if (result && rateLimits) {
             async.forEach(Object.keys(rateLimits),
               function(name, rateCB) {
                 var scope = mc.mm['rate-limit-scope'] + ':' + name;
                 var rateLimit = rateLimits[name];
                 ...
                 logger.debug('rateLimit start: ', name, rateLimit, match);
   ...
   promise.then(function() { contextsCB(null, matches); })
          .catch(function(err) {
            logger.debug(err);
...
function buildPreflowContext(EntryMatch, PathMatch, MethodMatch) {
  ...
  if (rateLimits) {
    rateLimit = rateLimits['x-ibm-unnamed-rate-limit'];
    delete rateLimits['x-ibm-unnamed-rate-limit'];
  }
  var plan = {
