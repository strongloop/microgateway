...
//console.log("apim-lookup PLAN:" + JSON.stringify(entry['plan-name']));
//console.log("apim-lookup PLAN RATELIMIT" + JSON.stringify(entry['plan-rate-limit']));
console.log("apim-lookup OPERATION RATELIMIT" + JSON.stringify(mm['operation-rate-limit']));
       ...
       return new Promise((resolve, reject) => {
         security.evalSecurity(ctx, mc.entry, reqs, defs, filters, (err, result) => {
           ...
           matches.push(match);
            ...
            var plan = match.plan;
            var rateLimit = plan && (plan.rateLimit || plan['rate-limit']);
            if (rateLimit) {
              var planId = plan.id;
              var limiter = rateLimiters[planId];
              if (!limiter) {
                limiter = rateLimiting(rateLimit);
                rateLimiters[planId] = limiter;
              }
              debug('rateLimit start: ', rateLimit, match);
