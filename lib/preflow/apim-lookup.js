...
function evalApikey(ctx, context, securityReq, securityDef, filters, callback) {
  ...
  if (securityDef.in === 'header') {
    ...
    if (securityDef.name === 'X-IBM-Client-Id') {
      if (filters.hdrClientId === context['client-id']) {
        result = true;
      }
    // Check fixed client secret header name/value
    } else if (securityDef.name === 'X-IBM-Client-Secret') {
      if (filters.hdrClientSecret === context['client-secret']) {
        result = true;
      }
    // Check arbitrary client ID header name/value
    } else if (securityDef['x-ibm-apikey'] && securityDef['x-ibm-apikey'] === 'clientid') {
      if (ctx.request.headers[securityDef.name.toLowerCase()] === context['client-id']) {
        result = true;
      }
    // Check arbitrary client secret header name/value
    } else if (securityDef['x-ibm-apikey'] && securityDef['x-ibm-apikey'] === 'clientsecret') {
      if (getHashedValue(ctx.request.headers[securityDef.name.toLowerCase()]) === context['client-secret']) {
        result = true;
      }
    }
...
function findContext(ctx, filters, body, contextsCB) {
  ...
  _.forEach(apiMatches, function(mc) { //mc = matchCandidate
    ...
    promise = promise.then(function() {
      return new Promise(function(resolve, reject) {
        security.evalSecurity(ctx, mc.entry, reqs, defs, filters, function(err, result, noSecurityReqs) {
          if (err) {
            reject(err);
            return;
          }
          ...
          match.testAppEnabled = mc.entry['test-app-enabled'];
