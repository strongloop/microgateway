...
function evalApikey(ctx, context, securityReq, securityDef, filters, callback) {
  ...
  if ((securityDef.in === 'header' &&
       ...
       ((securityDef.name === 'X-IBM-Client-Id' &&
        ...
        (securityDef['x-ibm-apikey'] && securityDef['x-ibm-apikey'] === 'clientid' &&
         ctx.request.headers[securityDef.name.toLowerCase()] === context['client-id']) ||
        // Check arbitrary client secret header name/value
        (securityDef['x-ibm-apikey'] && securityDef['x-ibm-apikey'] === 'clientsecret' &&
         getHashedValue(ctx.request.headers[securityDef.name.toLowerCase()]) === context['client-secret']))) ||
      (securityDef.in === 'query' &&
       ...
       ((securityDef.name === 'client_id' &&
        ...
        (securityDef['x-ibm-apikey'] && securityDef['x-ibm-apikey'] === 'clientid' &&
         queryParms[securityDef.name] === context['client-id']) ||
        // Check arbitrary client secret query parm name/value
        (securityDef['x-ibm-apikey'] && securityDef['x-ibm-apikey'] === 'clientsecret' &&
          queryParms[securityDef.name] &&
