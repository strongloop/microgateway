...
function apimcontextget (ctx, opts, cb) {
  ...
  var promise = snapshotDataRequest()
    .then(function(apis) {
    ...
    .then(function(apis) {
      logger.debug('apimcontextget final count: %d, value: %s',
            (Array.isArray(apis) ? apis.length : 0), JSON.stringify(apis));
      return [null, apis];
    })
    .catch(function(err) {
      // Handle error
      logger.debug('apimcontextget error: %s', err);
      return [err, null];
    });
    ...
    [
      function(callback) {
        dsc.getCurrentSnapshot()
          .then(function(ss) { snapshot = ss; callback(); })
          .catch(callback);
      },
...
function doOptimizedDataRequest(ctx, snapshot, filters) {
  ...
  var queryurl = 'http://' + host + ':' + port +
      '/api/optimizedData?filter=' + encodeURIComponent(JSON.stringify(queryfilter));
  ...
  return new Promise(function(resolve, reject) {
    request({url: queryurl}, function (error, response, body) {
      ...
      findContext(ctx, filters, body, function(err, localcontexts) {
        ...
        /*
        addFlows(localcontexts, function(err, contexts) {
