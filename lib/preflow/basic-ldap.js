...
function search (client, options) {
  return new Promise((resolve, reject) => {
    ...
    let opts = {
      filter:     options.searchFilter,
      scope:      options.searchScope,
      sizeLimit:  1,
      attributes: [ 'dn', 'sn', 'cn', 'mail', 'givenName' ]
    };
    let users = [];
    client.search(baseDn, opts, (err, res) => {
      ...
      res.on('searchEntry', entry => {
          ...
          debug('LDAP Entry found:', entry.object.dn);
      });
...
function authSearchDN (options, user, pass) {
  return new Promise((resolve, reject) => {
    ...
    options.searchFilter = generateDN(user, options.searchFilter);

    let p = options.authenticatedBind ? bind(client, options.bindDn, options.bindCredentials) : Promise.resolve();
    p.then(() => search(client, options))
      .then(users => {
        let len = users.length;
        if (len === 1)
          return bind(client, users[0].dn, pass);
        else if (len === 0) {
          let msg = `No user matching ${options.searchFilter} found`;
          debug(msg);
          throw new Error(msg);
        }
        else {
...
function authComposeDN (options, user, pass) {
  return new Promise((resolve, reject) => {
    ...
    bind(client, generateDN(user, options.bindDn), pass)
      .then(res => {
        debug('Authentication successful!');
        resolve(res);
        client.unbind(err => {
