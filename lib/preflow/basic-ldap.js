...
var logger = require('apiconnect-cli-logger/logger.js')
               .child({loc: 'microgateway:preflow:basic-ldap'});
...
function parseOptions (opts) {
  ...
  // ComposeDN - Attempt to bind with composed DN (may also be referred to as BindDN?)
    ...
    var bindsuffix  = ldapconf['bind-suffix'];
  ...
  return p.then(function() { return search(client, userauth); })
    .then(function(users) {
      ...
      if (len === 1) {
        ...
        var msg = 'No user matching ' + userauth.filter + ' found';
        logger.debug(msg);
        throw new Error(msg);
      }
      else {
        var msg = 'Too many results found: ' + users.map(function(u) { return u.dn; }).join(';');
        logger.debug(msg);
        throw new Error(msg);
      }
    })
