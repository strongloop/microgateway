 ...
 *
 *    TODO: Still need to figure out exactly how the values from the registry map into the ldapjs options
 *    From the registry:
...
function parseOptions (opts) {
  let ldapconf = opts['ldap-config'];

  const options = {
    url: undefined,             // host & port
    bindDn: undefined,          // authenticated-bind-admin-dn
    bindCredentials: undefined, // authenticated-bind-password
    bindProperty: 'dn',         // Not sure, maybe 'bind-prefix' + 'bind-suffix'
    searchBase: undefined,      // 'search-dn-base'
    searchScope: 'sub',         // 'search-dn-scope'
    searchFilter: undefined,    // 'search-filter', maybe 'search-dn-filter-prefix'+'search-filter'+'search-filter-dn-suffix' ?
    searchAttributes: undefined,// ??
    groupDnProperty: 'dn',      // 'static-group-dn' ?
    groupSearchBase: undefined, // ??
    groupSearchScope: 'sub',    // '
    groupSearchFilter: undefined,
    groupSearchAttributes: undefined
  };

  options.url = `${ldapconf['ssl'] ? 'ldaps' : 'ldap'}://${ldapconf['host']}:${ldapconf['port']}`;
  ...
  //options.bindProperty = ``;
  options.searchBase   = ldapconf['search-dn-base'] || 'dc=apim,dc=com';
  options.searchScope  = ldapconf['search-dn-scope'];
  options.searchFilter = ldapconf['search-filter'];
  return options;
}
