...
function parseOptions (opts) {
  ...
  else if (ldapconf['auth-method'] === 'composeDN') {
...
function authComposeDN (options, user, pass) {
  ...
  const generateDN = () => {
    const username = user.replace(/\*/g, '\\2a')
                         ...
                         .replace(/\0/g, '\\00')
                         .replace(/\//g, '\\2f');
    return options.bindDn.replace(/{{username}}/g, username);
  };
  return new Promise((resolve, reject) => {
    let client = ldap.createClient({ url: options.url, tlsOptions: options.tlsOptions });

    client.on('error', err => {
      logger.debug('Error when connecting to LDAP server:', err);
      reject(err);
    });

    client.bind(generateDN(), pass, (binderr, res) => {
      if (binderr)
        logger.debug('Error during LDAP bind:', binderr);

      client.unbind(unbinderr => {
        if (unbinderr)
          logger.debug('Error during LDAP unbind:', unbinderr);
