...
var evalFunctions = {
  apiKey: function(ctx, candidate, securityReq, securityDef, callback) {
    logger.warn('The default evalApikey handler is used. Will return false.');
    callback(false);
  },
  basic: function(ctx, candidate, securityReq, securityDef, callback) {
    logger.warn('The default evalBasic handler is used. Will return false.');
    callback(false);
  },
  oauth2: function(ctx, candidate, securityReq, securityDef, callback) {
...
function evalSecurity(ctx, candidate, securityReqs, securityDefs, callback) {
  var descriptor = candidate.doc;
  var apiName = descriptor['x-ibm-api-id'];
      ...
      async.forEach(Object.keys(securityReq),
        function(securityScheme, inAsyncCB) {
          ...
          if (securityDef) {
            ...
            evalFunctions[securityDef.type](ctx, candidate, securityReq, securityDef,
