...
var contentParser = {
  json: bodyParser.json( { strict: false, type: '*/*' }),
  text: bodyParser.text( { type: '*/*' }),
  urlencoded: bodyParser.urlencoded({ extended: true, type: '*/*' }),
  raw: bodyParser.raw( { type: '*/*'})
};
  ...
  return function(ctx, req, callback) {
    ...
    if (req.body) {
      ...
      if (!matchFound) {
        ...
        contentParser.raw(req, {}, function(err) {
