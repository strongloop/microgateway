...
module.exports = function createApimContextMiddleware(options) {
  ...
  return function(req, res, next) {
    var ctx = req.ctx;
    var _api = req.ctx._api;
    var sub = _api.subscription;
    var op = _api.doc.paths[_api.path][_api.method];
    if (sub) {
      var product = sub['plan-registration'].product;
      var plans = sub['plan-registration'].plans;
      var plan = sub['plan-registration'].plan;
    }
    // console.log(JSON.stringify(_api, null, 2))
    ctx.set('_.api', {
      id: _api.doc.info['x-ibm-name'],
      assembly: {
        assembly: _api.doc['x-ibm-configuration'].assembly
      },
      path: _api.path,
      operation: _api.method,
      "parameters": [],   // TODO
      responses: op.responses,
      // "subscription-id": "575f9724e4b06d109c285a2e",
      // "subscription-active": true,
      // "subscription-app-state": "ACTIVE"
    });
    ctx.set('config-snapshot-id', _api.doc.snapshotId);

    ctx.set('api', {
      document: _api.doc,
      name: _api.doc.info['x-ibm-name'],
      version: _api.doc.info.version,
      // properties: {},
      type: 'REST',
      state: _api.doc['x-ibm-api-state'],
      org:  _api.doc['x-ibm-organization'],
      operation: {
        path: _api.path
      }
    });
