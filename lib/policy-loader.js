 ...
 */
function create(policyDescriptions, policyCfgs) {
    // if (_.isArray(policyDescriptions)) {
    //     abspaths.forEach(function(item) {
    //         assert(_.isString(path) && pathM.isAbsolute(item.path),
...
function PolicyLoader(policyDescriptions, policyCfgs) {
        ...
        if (!exactMatchFound) {
          ...
          if (stat.isDirectory()) {
              try {
                  ...
                  policyCfg  =  localCfg[policyIndex] ||
    ...
    Object.defineProperty(this, '_paths', { value: policyDescriptions,
        writable: false,
        enumerable: false,
        configurable: false } );
}
...
exports.createMGLoader = function (options) {
    ...
    var policyDescriptions = getUserPolicies();
    logger.debug("TCB policy info", policyDescriptions);
    var someArray = [];
    someArray["foo" + ":" + "1.2.3"] = "17";

    return create(policyDescriptions, options || {});
};
