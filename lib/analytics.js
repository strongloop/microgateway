...
var logger = require('apiconnect-cli-logger/logger.js')
               .child({loc: 'apiconnect-microgateway:analytics'});
...
function processApiEvent(event) {
  ...
  logger.debug('Analytics Data:%s', data);
  pushQueue.push(data);
...
function batchPublish() {
  ...
  try {
    request.post(opts, function(err, res) {
      if (err) {
        return logger.debug('Cannot upload analytics events', err);
      }
      if (res.statusCode >= 400) {
        return logger.debug('Cannot upload analytics events: status code %s\n%s',
