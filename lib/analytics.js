...
module.exports = function sendAnalytics(opts) {
  return function captureActivity(req, res, next) {
    res.on('finish', function() {
      var duration = new Date() - req.ctx.request.start;
...
function buildApiEvent(req, res, duration) {
  ...
  return {
    ...
    bytesReceived: 0,
    apiVersion: '',
    orgId: req.ctx.api.org.id,
    envId: req.ctx.catalog.id,
    apiId: req.ctx.api.id || '',
    appId: req.ctx.client.app.id || '',
    resourceId: '',
    apiUser: '',
    responseBody: '',
    requestBody: '',
