      ...
      request
        ...
        .expect(function(res) {
            ...
            setTimeout(
              ...
              15000 // 15 seconds to ensure second snapshot begins
            );
          }
        );
    }
    ...
    function(done) {
      ...
      request
        ...
        .expect(function(res) {
            ...
            assert(res.body[1].id !== snaphostID);
            assert(res.body[1].id.length === 5); // ID's are strings of 5 characters
            assert(parseInt(res.body[1].id) >= 0); // ID's are >= 0
            assert(parseInt(res.body[1].id) < 65536); // ID's are < 65536
          }
        ).end(done);
    }
  );
